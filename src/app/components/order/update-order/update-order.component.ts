...
export class UpdateOrderComponent {
  ...
  loadDishesIntoFormArray(Dishes: Dish[]) {
    ...
    if( !Array.isArray(Dishes) || Dishes.length==0){
      ...
      for (var i=0; i< Dishes.length; i++) {
        dishFormArray.push(this.createDishFormGroup(Dishes[i], this.order.quantity[i]));
      }
  ...
  saveThirdStepData(formdata:FormGroup){
    ...
    dishesArr[0].forEach((add:any)=>{ 
      this.order.arrDishes.push(new Dish(this.addDishId++,add.dishName,add.price,add.img_path,parseInt(this.order.id.toString()),JSON.parse(add.isAvailable.toLowerCase())))
      this.order.quantity.push(add.quantity);
      this.order.orderAmount += add.price * add.quantity;
      idx++;
    })

    this.orderService.updateOrder(this.order, this.order.id).subscribe(data => {
